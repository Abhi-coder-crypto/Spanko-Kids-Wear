...
async function seedDatabase() {
  ...
  if (categories.length === 0) {
    ...
    // Subcategories
    const babyGirl = await storage.createCategory({ name: "Baby Girl", slug: "baby-girl", image: "...", parentId: baby.id });
    const babyBoy = await storage.createCategory({ name: "Baby Boy", slug: "baby-boy", image: "...", parentId: baby.id });

    // Types
    const pajamas = await storage.createCategory({ name: "Pajamas", slug: "pajamas", image: "...", parentId: babyGirl.id });
    const outfits = await storage.createCategory({ name: "Outfit Sets", slug: "outfits", image: "...", parentId: babyGirl.id });
